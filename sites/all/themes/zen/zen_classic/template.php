<?php
// $Id: template.php,v 1.1.2.16 2008/04/20 23:25:07 johnalbin Exp $
...
function zen_classic_phptemplate_variables($hooks, $vars = array()) {
  ...
  if (module_exists('advanced_forum')) {
    ...
    if (module_exists('clickpath')) {
      $vars = clickpath_addvars($hook, $vars);
...
function zen_classic_box($edit, $title = NULL) {
  if (module_exists('readpath') && $edit == t('Post new comment') && readpath_allowed_type(arg(1)) && !readpath_on_last_page()) {
    return;
  }
  else {
